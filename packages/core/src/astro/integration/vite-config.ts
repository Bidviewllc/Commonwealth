...
function linguiMacroPlugin(adminSourcePath: string, adminDistPath: string): Plugin {
	...
	// Normalize to forward slashes so path checks work on Windows (Vite IDs use /)
	const adminSourceNorm = adminSourcePath.replace(/\\/g, "/");
	...
	return {
		...
		resolveId(id, importer) {
			...
			if (!importer?.replace(/\\/g, "/").startsWith(adminSourceNorm)) return;
		...
		async transform(code, id) {
			if (!id.replace(/\\/g, "/").startsWith(adminSourceNorm) || !code.includes("@lingui")) return;
	...
	return {
		...
		resolve: {
			...
			alias: [
				...
				// Redirect Lingui macro packages to their runtime equivalents.
				// Macros are Babel compile-time — without the transform they crash
				// the browser. Runtime packages are identical in behaviour.
				{ find: "@lingui/react/macro", replacement: "@lingui/react" },
				{ find: "@lingui/core/macro", replacement: "@lingui/core" },
